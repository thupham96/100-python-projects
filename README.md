...
| 88   | Cafe & Wifi – Flask Web App                  | A full-stack Flask application with SQLite backend that displays cafes, supports search filters, and provides CRUD operations via both UI and REST API. | Done   |
| 89 | Flask To-Do – Simple Task Manager | A clean Flask CRUD app using SQLite + SQLAlchemy and Bootstrap; add, edit, toggle complete, and delete tasks. | Done |
...
| 92  | Color Extractor – Python Tkinter + Pillow App         | A desktop app that opens an image, previews it, and extracts the top 10 dominant colors with HEX/RGB values and percentage coverage. Allows easy copying of HEX codes to the clipboard.             | Done  |
...
| 97 | Flask E-commerce – Mini Shop | A simple online store built with Flask, SQLAlchemy, and Jinja: product grid, add-to-cart, update/remove items, running totals, and a checkout placeholder. Structured with blueprints and Bootstrap for styling. | Done |
